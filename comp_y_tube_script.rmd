...
comp_y_plot <- ggplot(data = plot.data, aes(x = condition,  group=condition)) +
  ...
  xlab("Condition") +
  ylab("Stabilised both trays") +
  scale_x_continuous(breaks = c(1, 2), labels = c("Control", "Test"), limits = c(0.5,2.5)) +
  ylim(0, 1) +
  theme_classic()

comp_y_plot 

ggsave(comp_y_plot , filename = "graphs/comp_y_plot.png", width = 7, height = 8, scale = 0.5)
...
#  geom_hline(yintercept=18.5, linetype="dashed", "black", size=0.75)+
 # scale_x_discrete(name ="Chimp ID")+
  scale_y_discrete(breaks=c(8,24),labels=c("Test", "Control"))+ #name= "Condition",
...
#  geom_hline(yintercept=18.5, linetype="dashed", "black", size=0.75)+
 # scale_x_discrete(name ="Chimp ID")+
  scale_y_discrete(breaks=c(8,24),labels=c("Control", "Test"))+ #name= "Condition",
  guides(color = guide_legend(order = 2),
         shape = guide_legend(order = 1))+
  coord_flip() 
